 ...
 */
function BaseRadio({ id = "radio", label = "", error = false, helperText = "", options = {}, inputProps = {}, ...rest }) {
  return (
    <Box m={2} sx={{ minWidth: 210 }}>
      <FormControl error={error}>
        <Box px={2} py={1} className={`outlined-input ${error ? "error" : ""}`}>
          <InputLabel id={`lbl-${id}`} shrink>
            {label}
          </InputLabel>
          <RadioGroup id={id} {...inputProps} {...rest}>
            {Object.entries(options).map(([key, val]) => (
              <FormControlLabel key={key} value={key} control={<Radio ref={inputProps.ref} />} label={val} />
            ))}
          </RadioGroup>
        </Box>
